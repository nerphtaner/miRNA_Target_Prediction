...
setwd('/home/node05/바탕화면/miRNA target performance/ProMISe/BRCA')
...
{
  #ROC and AUC calculated
  gpval = val[intersect(getGoldPositive(miR),rownames(table))]
  gnval = val[intersect(getGoldNegative(miR),rownames(table))]
  val = c(gpval, gnval)
  label = array(0, dim = length(val))
  label[1:length(gpval)]=1
  label = factor(label)
...
mRNA_data = read.delim(file = '/home/node05/바탕화면/miRNA target performance/ProMISe/BRCA/RSEM_genes_normalized_data/BRCA_RSEM_genes_normalized_data.txt',sep = '\t')
...
#edit sample list - miR
normal_names = c()
cancer_names = c()
normal_cancer_pair = c()
normal_cut = c()
cancer_cut = c()

for(tcga_sample in rownames(PARTmiR))
{
  if(as.numeric(unlist(strsplit(tcga_sample,split=""))[14])>0)
  {
    normal_names = c(normal_names,tcga_sample)
  }
}

for(tcga_sample in rownames(PARTmiR[!rownames(PARTmiR)%in%normal_names,]))
{
  for(id in normal_names)
  {
    if(paste(as.character(unlist(strsplit(id,split=''))[6:12]),collapse = '')==paste(as.character(unlist(strsplit(tcga_sample,split=''))[6:12]),collapse = '')&&paste(as.character(unlist(strsplit(tcga_sample,split=''))[14:15]),collapse = '')=='01')
    {
      cancer_names = c(cancer_names,tcga_sample)
    }
  }
}

for(sample_name in normal_names)
{
  normal_cut = c(normal_cut,paste(as.character(unlist(strsplit(sample_name,split=''))[1:12]),collapse = ''))
}
for(sample_name in cancer_names)
{
  cancer_cut = c(cancer_cut,paste(as.character(unlist(strsplit(sample_name,split=''))[1:12]),collapse = ''))
}
normal_names = normal_names[-47]

normal_cancer_pair = PARTmiR[c(normal_names,cancer_names),]

#edit sample list - mRNA
gene_pair = c()
normal_gene = c()
cancer_gene = c()
temp_names = c()

for(tcga_sample in rownames(PARTmRNA))
{
  if(as.numeric(unlist(strsplit(tcga_sample,split=""))[14])>0)
  {
    temp_names = c(temp_names,tcga_sample)
  }
}

for(tcga_sample in temp_names)
{
  for(id in normal_names)
  {
    if(paste(as.character(unlist(strsplit(id,split=''))[6:12]),collapse = '')==paste(as.character(unlist(strsplit(tcga_sample,split=''))[6:12]),collapse = ''))
    {
      normal_gene = c(normal_gene,tcga_sample)
    }
  }
}

for(tcga_sample in rownames(PARTmRNA[!rownames(PARTmRNA)%in%normal_gene,]))
{
  for(id in normal_names)
  {
    if(paste(as.character(unlist(strsplit(id,split=''))[6:12]),collapse = '')==paste(as.character(unlist(strsplit(tcga_sample,split=''))[6:12]),collapse = '')&&paste(as.character(unlist(strsplit(tcga_sample,split=''))[14:15]),collapse = '')=='01')
    {
      cancer_gene = c(cancer_gene,tcga_sample)
    }
  }
}

gene_pair = PARTmRNA[c(normal_gene,cancer_gene),]


#edit sample list - new Golds
temp_names = c()
normal_gene2 = c()
cancer_gene2 = c()
gene_pair2 = c()

for(tcga_sample in rownames(PARTmRNA_News))
{
  if(as.numeric(unlist(strsplit(tcga_sample,split=""))[14])>0)
  {
    temp_names = c(temp_names,tcga_sample)
...
for(tcga_sample in temp_names)
{
  for(id in normal_names)
  {
    if(paste(as.character(unlist(strsplit(id,split=''))[6:12]),collapse = '')==paste(as.character(unlist(strsplit(tcga_sample,split=''))[6:12]),collapse = ''))
    {
      normal_gene2 = c(normal_gene2,tcga_sample)
    }
  }
}

for(tcga_sample in rownames(PARTmRNA[!rownames(PARTmRNA_News)%in%normal_gene2,]))
{
  for(id in normal_names)
  {
    if(paste(as.character(unlist(strsplit(id,split=''))[6:12]),collapse = '')==paste(as.character(unlist(strsplit(tcga_sample,split=''))[6:12]),collapse = '')&&paste(as.character(unlist(strsplit(tcga_sample,split=''))[14:15]),collapse = '')=='01')
    {
      cancer_gene2 = c(cancer_gene2,tcga_sample)
    }
  }
}

gene_pair2 = PARTmRNA_News[c(normal_gene2,cancer_gene2),]

PARTmiR = normal_cancer_pair
PARTmRNA = gene_pair
PARTmRNA_News = gene_pair2
